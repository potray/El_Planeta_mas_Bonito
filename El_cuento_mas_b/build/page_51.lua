...
function new() 
    ...
    -- Audio callings 
    local hermana_page42 =  audio.loadStream( audioDir.."hermana_page42.mp3") 
    local hormiga_page42 =  audio.loadStream( audioDir.."hormiga_page42.mp3") 
    ...
    local drawScreen = function() 
       ...
       Navigation.new("page", { backColor = {125, 125, 125}, anim=1, timer=1,  totPages = numPages, curPage = curPage, thumbW = 200, thumbH = 125, alpha = 0, imageDir = imgDir, dire = "bottom", audio={{ 1, "hermana_page42"},{ 2, "hormiga_page42"},} } ) 
       Navigation.hide() 
...
local hasCola = kwkVarCheck("hasCola") 
       ...
       -- Botones positioning 
       Botones = display.newImageRect( imgDir.. "p51_botones.png", 2176, 864 ); 
       ...
       -- Text1 positioning 
       Text1 = display.newImageRect( imgDir.. "p51_text1.png", 676, 51 ); 
       ...
       -- Text2 positioning 
       Text2 = display.newImageRect( imgDir.. "p51_text2.png", 1381, 108 ); 
       Text2.x = 1282; Text2.y = 120; Text2.alpha = 1; Text2.oldAlpha = 1 
       ...
       function hideInventory(event) 
            transitionStash.newTransition_905 = transition.to( kwkRectInv, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_905 = transition.to( kwkInvBiberon, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_906 = transition.to( kwkInvTijeras, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_906 = transition.to( kwkInvRosa, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_906 = transition.to( kwkInvErizo, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_906 = transition.to( kwkInvMaquina, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_907 = transition.to( kwkInvCelo, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_907 = transition.to( kwkInvGuitarra, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_907 = transition.to( kwkInvCola, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_907 = transition.to( kwkInvCerrar, {alpha=0, time=0, delay=0}) 
           InventoryShown = false
          saveKwikVars({"InventoryShown",false}) 
       end 

       function showInventory(event) 
            transitionStash.newTransition_925 = transition.to( kwkRectInv, {alpha=kwkRectInv.oldAlpha, time=0, delay=0}) 
         if (hasBiberon == true) then 
            transitionStash.newTransition_926 = transition.to( kwkInvBiberon, {alpha=kwkInvBiberon.oldAlpha, time=0, delay=0}) 
         end 
         if (hasTijeras == true) then 
            transitionStash.newTransition_927 = transition.to( kwkInvTijeras, {alpha=kwkInvTijeras.oldAlpha, time=0, delay=0}) 
         end 
         if (hasMaquina == true) then 
            transitionStash.newTransition_927 = transition.to( kwkInvMaquina, {alpha=kwkInvMaquina.oldAlpha, time=0, delay=0}) 
         end 
         if (hasRosa == true) then 
            transitionStash.newTransition_928 = transition.to( kwkInvRosa, {alpha=kwkInvRosa.oldAlpha, time=0, delay=0}) 
         end 
         if (hasErizo == true) then 
            transitionStash.newTransition_929 = transition.to( kwkInvErizo, {alpha=kwkInvErizo.oldAlpha, time=0, delay=0}) 
         end 
         if (hasCelo == true) then 
            transitionStash.newTransition_930 = transition.to( kwkInvCelo, {alpha=kwkInvCelo.oldAlpha, time=0, delay=0}) 
         end 
         if (hasGuitarra == true) then 
            transitionStash.newTransition_931 = transition.to( kwkInvGuitarra, {alpha=kwkInvGuitarra.oldAlpha, time=0, delay=0}) 
         end 
         if (hasCola == true) then 
            transitionStash.newTransition_931 = transition.to( kwkInvCola, {alpha=kwkInvCola.oldAlpha, time=0, delay=0}) 
         end 
           InventoryShown = true
          saveKwikVars({"InventoryShown",true}) 
            transitionStash.newTransition_949 = transition.to( kwkInvDestacado, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_949 = transition.to( kwkInvCerrar, {alpha=kwkInvCerrar.oldAlpha, time=0, delay=0}) 
       end 

       function Anim(event) 
            transitionStash.newTransition_950 = transition.to( Text2, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_950 = transition.to( kwkNavnext, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_950 = transition.to( Text2, {alpha=Text2.oldAlpha, time=1000, delay=1500}) 
            transitionStash.newTransition_950 = transition.to( kwkNavnext, {alpha=kwkNavnext.oldAlpha, time=1000, delay=3000}) 
       end 

       function AudioAction(event) 
         if (Audio == true) then 
           audio.setVolume(1, {channel=1} ) 
           audio.play( hermana_page42, {channel=1, loops = 0 } ) 
           audio.setVolume(1, {channel=2} ) 

          local myClosure_hormiga_page42 = function() 
             audio.play(hormiga_page42, {  channel=2, loops=0 } ) 
          end 
          timerStash.newTimer_973 = timer.performWithDelay(1500, myClosure_hormiga_page42, 1) 
         end 
       end 
      ...
      --End Actions (functions) 
       ...
       function btnPrevPage(self) 
            local myClosure_switch = function() 
                dispose(); director:changeScene( "page_49", "moveFromLeft" ) 
            end 
            timerStash.newTimer_041 = timer.performWithDelay(0, myClosure_switch, 1) 
       end 

       function btnNextPage(self) 
         if (Fav03Completed == false) then 
            local myClosure_switch = function() 
                dispose(); director:changeScene( "page_52", "moveFromRight" ) 
            end 
            timerStash.newTimer_042 = timer.performWithDelay(0, myClosure_switch, 1) 
         else 
            local myClosure_switch = function() 
                dispose(); director:changeScene( "page_53", "moveFromRight" ) 
            end 
            timerStash.newTimer_048 = timer.performWithDelay(0, myClosure_switch, 1) 
         end 
       end 
 ...
 audio.dispose(hermana_page42); hermana_page42 = nil 
          if audio.isChannelActive ( 2 ) then 
   audio.stop(2); 
 end 
 audio.dispose(hormiga_page42); hormiga_page42 = nil 
       end 
