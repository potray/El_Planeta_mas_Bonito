...
function new() 
    ...
    -- Audio callings 
    local narracion_page4 =  audio.loadStream( audioDir.."narracion_page41.mp3") 
...
local hasCola = kwkVarCheck("hasCola") 
       ...
       function hideInventory(event) 
            transitionStash.newTransition_304 = transition.to( kwkRectInv, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_304 = transition.to( kwkInvBiberon, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_305 = transition.to( kwkInvTijeras, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_305 = transition.to( kwkInvRosa, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_305 = transition.to( kwkInvErizo, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_305 = transition.to( kwkInvMaquina, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_306 = transition.to( kwkInvCelo, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_306 = transition.to( kwkInvGuitarra, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_306 = transition.to( kwkInvCola, {alpha=0, time=0, delay=0}) 
       ...
       function showInventory(event) 
            transitionStash.newTransition_325 = transition.to( kwkRectInv, {alpha=kwkRectInv.oldAlpha, time=0, delay=0}) 
         if (hasBiberon == true) then 
            transitionStash.newTransition_326 = transition.to( kwkInvBiberon, {alpha=kwkInvBiberon.oldAlpha, time=0, delay=0}) 
         end 
         if (hasTijeras == true) then 
            transitionStash.newTransition_327 = transition.to( kwkInvTijeras, {alpha=kwkInvTijeras.oldAlpha, time=0, delay=0}) 
         end 
         if (hasMaquina == true) then 
            transitionStash.newTransition_327 = transition.to( kwkInvMaquina, {alpha=kwkInvMaquina.oldAlpha, time=0, delay=0}) 
         end 
         if (hasRosa == true) then 
            transitionStash.newTransition_328 = transition.to( kwkInvRosa, {alpha=kwkInvRosa.oldAlpha, time=0, delay=0}) 
         end 
         if (hasErizo == true) then 
            transitionStash.newTransition_329 = transition.to( kwkInvErizo, {alpha=kwkInvErizo.oldAlpha, time=0, delay=0}) 
         end 
         if (hasCelo == true) then 
            transitionStash.newTransition_330 = transition.to( kwkInvCelo, {alpha=kwkInvCelo.oldAlpha, time=0, delay=0}) 
         end 
         if (hasGuitarra == true) then 
            transitionStash.newTransition_330 = transition.to( kwkInvGuitarra, {alpha=kwkInvGuitarra.oldAlpha, time=0, delay=0}) 
         end 
         if (hasCola == true) then 
            transitionStash.newTransition_331 = transition.to( kwkInvCola, {alpha=kwkInvCola.oldAlpha, time=0, delay=0}) 
         end 
           InventoryShown = true
          saveKwikVars({"InventoryShown",true}) 
            transitionStash.newTransition_351 = transition.to( kwkInvDestacado, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_351 = transition.to( kwkInvCerrar, {alpha=kwkInvCerrar.oldAlpha, time=0, delay=0}) 
       end 

       function hideCola(event) 
         if (hasCola == true) then 
         end 
            transitionStash.newTransition_352 = transition.to( kwkCola, {alpha=0, time=0, delay=0}) 
       end 
       ...
       function Anim(event) 
            transitionStash.newTransition_363 = transition.to( Botones, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_363 = transition.to( kwkNavnext, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_364 = transition.to( Botones, {alpha=Botones.oldAlpha, time=1000, delay=2500}) 
         if (Favor5Visitado == false) then 
            transitionStash.newTransition_364 = transition.to( kwkNavnext, {alpha=kwkNavnext.oldAlpha, time=1000, delay=2500}) 
         end 
       end 
      ...
      --End Actions (functions) 
       ...
       function btnPrevPage(self) 
            ...
            timerStash.newTimer_427 = timer.performWithDelay(0, myClosure_switch, 1) 
       end 
       ...
       function btnNextPage(self) 
         if (Preg05Completed == false) then 
            ...
            timerStash.newTimer_436 = timer.performWithDelay(0, myClosure_switch, 1) 
         end 
       end 
