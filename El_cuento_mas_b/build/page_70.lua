...
function new() 
    ...
    -- Audio callings 
    local narracion_page5 =  audio.loadStream( audioDir.."narracion_page57.mp3") 
    ...
    local drawScreen = function() 
       ...
       -- Layer names 
       local kwkFondo_62  
       local Mama_gafas_conc  
       local Hija_Gafas_conc  
       local Text1  
       local kwkCorazon  
...
local hasCola = kwkVarCheck("hasCola") 
       ...
       -- Mama_gafas_conc positioning 
       Mama_gafas_conc = display.newImageRect( imgDir.. "p70_mama_gafas_conc.png", 701, 1343 ); 
       Mama_gafas_conc.x = 1028; Mama_gafas_conc.y = 927; Mama_gafas_conc.alpha = 1; Mama_gafas_conc.oldAlpha = 1 
       Mama_gafas_conc.oriX = Mama_gafas_conc.x; Mama_gafas_conc.oriY = Mama_gafas_conc.y 
       Mama_gafas_conc.name = "Mama_gafas_conc" 
       menuGroup:insert(Mama_gafas_conc); menuGroup.Mama_gafas_conc = Mama_gafas_conc 

       -- Hija_Gafas_conc positioning 
       Hija_Gafas_conc = display.newImageRect( imgDir.. "p70_hija_gafas_conc.png", 605, 1104 ); 
       Hija_Gafas_conc.x = 1590; Hija_Gafas_conc.y = 860; Hija_Gafas_conc.alpha = 1; Hija_Gafas_conc.oldAlpha = 1 
       Hija_Gafas_conc.oriX = Hija_Gafas_conc.x; Hija_Gafas_conc.oriY = Hija_Gafas_conc.y 
       Hija_Gafas_conc.name = "Hija_Gafas_conc" 
       menuGroup:insert(Hija_Gafas_conc); menuGroup.Hija_Gafas_conc = Hija_Gafas_conc 

       -- Text1 positioning 
       Text1 = display.newImageRect( imgDir.. "p70_text1.png", 1508, 282 ); 
       Text1.x = 1279; Text1.y = 141; Text1.alpha = 1; Text1.oldAlpha = 1 
       ...
       function hideInventory(event) 
            transitionStash.newTransition_866 = transition.to( kwkRectInv, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_866 = transition.to( kwkInvBiberon, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_867 = transition.to( kwkInvTijeras, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_867 = transition.to( kwkInvRosa, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_867 = transition.to( kwkInvErizo, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_867 = transition.to( kwkInvMaquina, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_868 = transition.to( kwkInvCelo, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_868 = transition.to( kwkInvGuitarra, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_868 = transition.to( kwkInvCola, {alpha=0, time=0, delay=0}) 
            transitionStash.newTransition_868 = transition.to( kwkInvCerrar, {alpha=0, time=0, delay=0}) 
           InventoryShown = false
          saveKwikVars({"InventoryShown",false}) 
       end 

       function showInventory(event) 
            transitionStash.newTransition_887 = transition.to( kwkRectInv, {alpha=kwkRectInv.oldAlpha, time=0, delay=0}) 
         if (hasBiberon == true) then 
            transitionStash.newTransition_887 = transition.to( kwkInvBiberon, {alpha=kwkInvBiberon.oldAlpha, time=0, delay=0}) 
         end 
         if (hasTijeras == true) then 
            transitionStash.newTransition_888 = transition.to( kwkInvTijeras, {alpha=kwkInvTijeras.oldAlpha, time=0, delay=0}) 
         end 
         if (hasMaquina == true) then 
            transitionStash.newTransition_889 = transition.to( kwkInvMaquina, {alpha=kwkInvMaquina.oldAlpha, time=0, delay=0}) 
         end 
         if (hasRosa == true) then 
            transitionStash.newTransition_890 = transition.to( kwkInvRosa, {alpha=kwkInvRosa.oldAlpha, time=0, delay=0}) 
         end 
         if (hasErizo == true) then 
            transitionStash.newTransition_890 = transition.to( kwkInvErizo, {alpha=kwkInvErizo.oldAlpha, time=0, delay=0}) 
         end 
         if (hasCelo == true) then 
            transitionStash.newTransition_891 = transition.to( kwkInvCelo, {alpha=kwkInvCelo.oldAlpha, time=0, delay=0}) 
         end 
         if (hasGuitarra == true) then 
            transitionStash.newTransition_892 = transition.to( kwkInvGuitarra, {alpha=kwkInvGuitarra.oldAlpha, time=0, delay=0}) 
         end 
         if (hasCola == true) then 
            transitionStash.newTransition_892 = transition.to( kwkInvCola, {alpha=kwkInvCola.oldAlpha, time=0, delay=0}) 
         end 
           InventoryShown = true
          saveKwikVars({"InventoryShown",true}) 
            transitionStash.newTransition_911 = transition.to( kwkInvDestacado, {alpha=0, time=0, delay=0}) 
      ...
      --End Actions (functions) 
       ...
       function btnNextPage(self) 
            local myClosure_switch = function() 
                dispose(); director:changeScene( "page_71", "moveFromRight" ) 
            end 
       ...
       function btnPrevPage(self) 
            local myClosure_switch = function() 
                dispose(); director:changeScene( "page_69", "moveFromLeft" ) 
            end 
            timerStash.newTimer_986 = timer.performWithDelay(0, myClosure_switch, 1) 
       end 
